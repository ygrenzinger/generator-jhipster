...
<% for (fieldId in fields) {
    ...
    private static final LocalDate <%=updatedValueName %> = ZonedDateTime.now(ZoneId.systemDefault());<% } else if (fields[fieldId].fieldType == 'DateTime') { %>

    private static final DateTime <%=defaultValueName %> = ZonedDateTime.ofInstant(Instant.ofEpochMilli(0L), ZoneId.systemDefault());
    private static final DateTime <%=updatedValueName %> = ZonedDateTime.now(ZoneId.systemDefault());
    private static final String <%=defaultValueName %>_STR = dateTimeFormatter.format(<%= defaultValueName %>);<% } else if (fields[fieldId].fieldType == 'Boolean') { %>
